...
public final class RawWindowsVolume extends AbstractRawVolume {

	static Kernel32 kernel32 = Kernel32.INSTANCE;
	private HANDLE hDevice;

	public RawWindowsVolume(String driveLetter, boolean readOnly, long totalSpace) throws Exception {
		super(readOnly);
	...
	public void close() {
		super.close();
		boolean success = Kernel32.INSTANCE.FlushFileBuffers(hDevice
				.getPointer());
		boolean success1 = Kernel32.INSTANCE.CloseHandle(hDevice);
		System.out.println("Success flush: " + (success && success1));
		this.closed = true;
	}

	private boolean seek(long offset) {
		boolean success = Kernel32.INSTANCE.SetFilePointerEx(hDevice, offset,
				null, new DWORD(0));
		if (success) {
	...
	protected int readByteBuffer(long offset, ByteBuffer lpBuffer) {
		...
		boolean success = Kernel32.INSTANCE.ReadFile(hDevice, wrapDirect,
	...
	@Override
	protected boolean writeByteArray(long offset, byte[] data) {
		...
		boolean success = Kernel32.INSTANCE.WriteFile(hDevice, data,
