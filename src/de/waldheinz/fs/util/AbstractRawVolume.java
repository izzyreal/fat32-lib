...
public abstract class AbstractRawVolume implements BlockDevice {
	...
	public AbstractRawVolume(boolean readOnly) throws Exception {
	...
	@Override
	public void read(long devOffset, ByteBuffer destination) throws IOException {
		checkClosed();
...
//		System.out.println("buffer remaining: " + destination.remaining());
		if ((devOffset + toRead) > getSize()) throw new IOException("reading past end of device");
	...
	private boolean writeByteBuffer(long offset, ByteBuffer buffer) {
	...
	protected abstract boolean writeByteArray(long offset, byte[] data);
