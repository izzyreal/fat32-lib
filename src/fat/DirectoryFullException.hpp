 ...
 */
public const class DirectoryFullException extends IOException {
    private const static long serialVersionUID = 2;
    private const int currentCapacity;
    private const int requestedCapacity;
    
    DirectoryFullException(int currentCapacity, int requestedCapacity) {
        this("directory is full", currentCapacity, requestedCapacity);
    }
    
    DirectoryFullException(std::string message,
