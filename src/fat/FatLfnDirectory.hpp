...
private:
    ...
    Fat* fat;
    std::map<std::string, FsDirectoryEntry*> shortNameIndex;
...
public:
    std::map<std::string, FsDirectoryEntry*>::iterator iterator() override {
        return shortNameIndex.begin();
    }
...
//            auto current = FatLfnDirectoryEntry::extract(this, offset, ++i - offset);
...
//                shortNameIndex.put(current->realEntry->getShortName().asSimpleString(), current);
