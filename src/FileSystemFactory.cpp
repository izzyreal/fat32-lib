...
import de.waldheinz.fs.fat.AkaiFatFileSystem;
...
public class FileSystemFactory {
     ...
     * @throw (std::exception) on read error, or if the file system type could
     *      not be determined
     */
    public static FileSystem create(BlockDevice device, bool readOnly)
            throw (std::exception) {
            
        return FatFileSystem.read(device, readOnly);
    }

    public static FileSystem createAkai(BlockDevice device, bool readOnly)
            throw (std::exception) {
